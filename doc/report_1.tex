...
If given a data set $Y$ of $n$ samples and the set is assumed to be on the form
\[
Y = f(X)+ \varepsilon 
\]
where $\varepsilon \sim N(0,\ \sigma ^2)$ and $f$ is some function of a design matrix $X\in R^{n\times p}$, then it is possible by using linear regression to fit a linear model of $p$ degrees with outcome $\hat{Y}$ on the form
\[
\hat{Y} = \bm{X}^T\hat{\beta},
\]
with $\hat{\beta}$ being a $(p\times 1)$ vector containing linear regression parameters.
...
\end{align*}
 which in this project is defined on $x,\ y \in [0,1]$. 




it can be useful to consider the mean squared error (MSE), defined as 
\begin{equation}
