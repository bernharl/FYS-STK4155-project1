...
$\lambda^\text{lasso}_\text{T}$ & $10$ & $10^{4}$\\ \hline 
\end{tabular}}
\end{table}
...
The Franke's function EPE plotted against various degrees of the OLS regression is shown in Figure \ref{fig:bias_ols_Franke}.  There appears to be a minimum for the test data EPE at polynomial degree = 5, so we have chosen this as the polynomial degree for all models of the Franke's function data. Figure \ref{fig:bias_ridge_Franke} and \ref{fig:bias_lasso_Franke} show the EPE for varying $\lambda^\text{ridge}$ and $\lambda^\text{lasso}$ respectively, using a fifth order polynomial fit. Due to the variance in the Franke's function from the added  noise, it is not clear which values are optimal to use for the linear fits. 


\begin{table}[H]
\caption{Table of the $R^2$ scores for both training and test sets for all three regression methods. A fifth order polynomial model was evaluated for the Franke's function data, while a fifth order polynomial was evaluated for the terrain data. \vspace{2 pt}}
\label{tab:R2}
...
\begin{figure}[H]
    ...
    \caption{OLS regression parameters $\bm{\hat{\beta}}^{\text{OLS}}_F$  of a fifth order two-dimensional polynomial model  of the generated Franke's function with noise of $\sigma^2=0.1$, plotted with $68\%$ confidence intervals.}
