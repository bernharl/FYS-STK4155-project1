...
\noindent
For OLS, we run k-fold cross-validation on increasing polynomial degrees to get a good estimate of how the EPE varies with model complexity, with degrees ranging from 0 to 10. The best polynomial degree is determined by making a compromise between EPE and computational cost, as we do not necessarily wish to increase the polynomial degree, and subsequently the computational cost, for a minimal improvement in the results. This is especially important for larger data sets, such as the terrain data. The EPE for both training and test data is shown in Figure \ref{fig:bias_ols_Franke} for Franke's function and Figure \ref{fig:bias_ols_terrain} for terrain data.

As cross-validation can be computationally expensive, we assume that the optimal polynomial degree for OLS will also be optimal for Ridge and Lasso regression. The hyperparameter $\lambda$ is decided similarly to the polynomial degree; by cross-validating  Ridge and Lasso regression for a range of hyperparameters. However, as the penalty terms for Ridge and Lasso are different, as shown in Equation (\ref{eq:argminbeta_ridge}) and (\ref{eq:argminbeta_lasso}), the shrinkage is not the same for the methods and we need to tune their hyperparameters individually, hereon referred to as $\lambda^\text{ridge}$ and $\lambda^\text{lasso}$ respectively.

\section{Results}
\subsection{Franke's function data}

\begin{table}[H]
\caption{Table of the $R^2$ scores for training and test sets using the three regression methods on Franke's function with standard deviation $\sigma=1$. As the generated data contains large amounts of stochastic noise, we have included results from three runs.}
\label{tab:R2_stddev=1}
\centering
\begin{tabular}{|c|c|r|} \hline 
Method & $R^2$ training & $R^2$ test \\ \hline
\multirow{3}{*}{OLS} & 0.183& $-0.128$\\ 
											& 0.126 & $-0.102$ \\ 
											& 0.158   & 0.106\\ \cline{1-3}
\multirow{3}{*}{Ridge} & 0.0762& 0.0839\\ 
											& 0.151  & 0.0568 \\ 
											& 0.0977& 0.0965 \\ \cline{1-3}
\multirow{3}{*}{Lasso} & 0.0582& 0.0992\\ 
											& 0.175   & $-0.0914$ \\ 
											& 0.143  & 0.0229 \\ \cline{1-3}
\end{tabular}
\end{table}
...
\begin{tabular}{|c|c|c|c|} \hline
	...
	\multirow{3}{*} {\makecell[l]{Franke's  \\function}}&  OLS  &  0.886 & 0.827 \\ \cline{2-4}
																		& Ridge & 0.773 &  0.693 \\ \cline{2-4}
																		& Lasso & 0.789 & 0.808 \\ \hline
	\multirow{3}{*}{Terrain} 					&  OLS  &  0.752 & 0.752 \\ \cline{2-4}
																		& Ridge & 0.750 & 0.750 \\ \cline{2-4}
...
\begin{figure}[H]
    ...
    \caption{OLS regression parameters $\bm{\hat{\beta}}^{\text{OLS}}_F$  of a fifth order polynomial model  to the Franke function, plotted with $68\%$ confidence intervals.}
...
\begin{figure}[H]
    ...
    \caption{OLS regression parameters $\bm{\hat{\beta}}^{\text{OLS}}_T$  of a fifth order polynomial model  of the terrain data from Lake Tanganyika, here plotted with $68\%$ confidence intervals.}
